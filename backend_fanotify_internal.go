...
// fanotifyAction represents an event / operation on a particular file/directory
...
func fanotifyMarkMaskValid(mask uint64) error {
	...
	if isSet(mask, unix.FAN_MARK_MOUNT) &&
		(isSet(mask, unix.FAN_CREATE) ||
...
func (w *FanotifyWatcher) readEvents() error {
...
func (a fanotifyAction) Has(actions fanotifyAction) bool {
	return a&actions == actions
}

// Or appends the specified action to the set of actions to watch for
func (a fanotifyAction) Or(actions fanotifyAction) fanotifyAction {
	return a | actions
}

func (a fanotifyAction) toOp() Op {
	var op Op
	if a.Has(unix.FAN_CREATE) || a.Has(unix.FAN_MOVED_TO) {
		op |= Create
	}
	if a.Has(unix.FAN_DELETE) || a.Has(unix.FAN_DELETE_SELF) {
		op |= Remove
	}
	if a.Has(unix.FAN_MODIFY) {
		op |= Write
	}
	if a.Has(unix.FAN_MOVE_SELF) || a.Has(unix.FAN_MOVED_FROM) {
		op |= Rename
	}
	if a.Has(unix.FAN_ATTRIB) {
		op |= Chmod
	}
	if a.Has(unix.FAN_CLOSE_WRITE) || a.Has(unix.FAN_CLOSE_NOWRITE) {
		op |= Close
	}
